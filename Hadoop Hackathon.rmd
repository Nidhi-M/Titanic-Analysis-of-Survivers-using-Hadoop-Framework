###Question 1
Using HDFS:
a.PUT 3 files (train.tsv, test.tsv, titanic_master.tsv) into HDFS under /user/hue directory ( *.tsv files under /STAGING/TITANIC/ )
...
hdfs dfs -cat /user/hue/train.tsv | head
hdfs dfs -cat /user/hue/test.tsv | head
hdfs dfs -cat /user/hue/titanic_master.tsv | head


###Question 2
Using Hive:
a)Create 2 tables (TRAIN, TEST) and confirm can query. Using Cube, find passenger counts who Survived based on 'pclass' in TRAIN table.
b)Also query to find how many boys under 16 years younger Survived
...
--3b - From Hive, query all 3 tables and confirm have the correct row count
SELECT * from train; --891 rows
SELECT * from test; -- 418 rows
SELECT * from titanic_master; --1309 rows


--3c -1. From TRAIN, use CUBE to find passenger counts 
...
###Question 3:
Using Spark:
a)Create a Temporary Table and query to find the counts of how many Males and Females were in the TITANIC_MASTER table
...
Question 4:
Using Presto:   
a)Who paid more than 250 pounds for their 'fare'?
b)Which 'embarked' location had the most passengers?
...
a)Create 2 tables (TRAIN, TEST) and confirm can query. Using Group By, find passenger counts who survived based on 'pclass' in TRAIN table
b)Did Captain go down with the ship?
